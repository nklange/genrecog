functions {
  ...
  */
   ...
   */
   vector cumulative_probit_pmf(real mu, real disc, vector thres, real mix) {
     ...
     if(mix){
       ...
       for(y in 1:(nthres+1)){
         if (y == 1) {
           p[y] = disc * Phi(thres[1] - mu) + (1 - disc) * Phi(thres[1]);
         } else if (y == nthres + 1) {
           p[y] = disc * (1 - Phi(thres[nthres] - mu)) + (1 - disc) * (1 - Phi(thres[nthres]));
         } else {
...
model {
  ...
  if (!prior_only) {
    ...
    for (n in 1:N){
        ...
        theta[n,] = to_row_vector(cumulative_probit_pmf(mu[n], disc[n], to_vector(Intercept[n,]), X_disc[n,1]));
    }
    for (n in 1:N) {
