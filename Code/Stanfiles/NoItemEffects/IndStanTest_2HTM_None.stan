functions {
  ...
  */
   ...
   */
   vector cumulative_probit_pmf(real DO, real DN, real go, vector s_m, vector a_m_o, vector a_m_n, real old) {
     ...
     if(old){
      for(y in 1:C){
        if (y > Cold){
          p[y] = DO * s_m[C + 1 - y] + (1-DO) * go * a_m_o[C + 1 - y];
        } else if (y <= Cold){
          p[y] = (1-DO) * (1-go) * a_m_n[y];    
        }
       ...
       for(y in 1:C){
        if (y <= Cold) {
          p[y] = DN * s_m[y] + (1-DN) * (1-go) * a_m_n[y];
        } else {
          p[y] = (1-DN) * (go) * a_m_o[C + 1 - y];    
        }
...
model {
  ...
  if (!prior_only) {
    ...
    for (n in 1:N){
        ...
        DN[n] =  Phi(DN[n]);
        go[n] =  Phi(go[n]);
        theta[n,] = to_row_vector(cumulative_probit_pmf(DO[n], DN[n], go[n], s_m[J_1[n],], a_m_o[J_1[n],], a_m_n[J_1[n],], X_DO[n,1]));
    }
    for (n in 1:N) {
  ...
  target += normal_lpdf(b_DO | 0,1);
  target += normal_lpdf(b_DN | 0,1);
