functions {
  ...
  */
   ...
   */
   real cumulative_probit_pmf(int y, real mu, real disc, vector thres) {
     ...
     if (y == 1) {
       ...
       p = disc + (1.0 - disc) * (1 - Phi(thres[nthres] - mu));
     } else {
...
model {
  ...
  if (!prior_only) {
    ...
    for (n in 1:N){
        ...
        target += cumulative_probit_pmf(Y[n], mu[n], disc[n], to_vector(Intercept[n,]));
    }
  }
