functions {
  ...
  */
   ...
   */
   real cumulative_probit_pmf(int y, real mu, real disc, vector thres, real mix, real nonattmu) {
     ...
     if(mix){
         ...
         if (y == 1) {
           p = disc * Phi(thres[1] - mu) + (1 - disc) * Phi(thres[1] - nonattmu);
         } else if (y == nthres + 1) {
           p = disc * (1 - Phi(thres[nthres] - mu)) + (1 - disc) * (1 - Phi(thres[nthres] - nonattmu));
         } else {
           p = disc * (Phi(thres[y] - mu) -
           Phi(thres[y - 1] - mu)) +
...
transformed parameters {
  ...
  r_1_gamma = r_1[,(G + G_disc + G_nonattmu):M_1];
...
model {
  ...
  if (!prior_only) {
    ...
    for (n in 1:N) {
      ...
      nonattmu[n] += dot_product(r_1_nonattmu[J_1[n],], Z_nonattmu[n,]) + r_2_nonattmu[J_2[n]] * Z_2_nonattmu[n];
    }
    //for (n in 1:N) {
    ...
    for (n in 1:N){
        ...
        target += cumulative_probit_pmf(Y[n], mu[n], disc[n], to_vector(Intercept[n,]), X_disc[n,1], nonattmu[n]*mu[n]);
    }
  }
