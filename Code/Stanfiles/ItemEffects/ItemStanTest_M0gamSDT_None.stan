functions {
  ...
  */
   ...
   */
   real cumulative_probit_pmf(int y, real mu, real disc, vector thres, real mix) {
     ...
     if(mix){
         ...
         if (y == 1) {
           p = disc * Phi(thres[1] - mu) + (1 - disc) * Phi(thres[1]);
         } else if (y == nthres + 1) {
           p = disc * (1 - Phi(thres[nthres] - mu)) + (1 - disc) * (1 - Phi(thres[nthres]));
         } else {
...
model {
  ...
  if (!prior_only) {
    ...
    for (n in 1:N){
        ...
        target += cumulative_probit_pmf(Y[n], mu[n], disc[n], to_vector(Intercept[n,]), X_disc[n,1]);
    }
  }
