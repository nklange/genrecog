...
## Item Effects, Random Crit models
## resultant files in ProcessedPredictions/ItemEffectsRandomCrit/ folder
...
EVSDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/Full/EVSDT/", full.names = TRUE)
UVSDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/Full/UVSDT/", full.names = TRUE)
DPSDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/Full/DPSDT/", full.names = TRUE)
M0SDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/Full/M0SDT/", full.names = TRUE)
MASDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/Full/MASDT/", full.names = TRUE)
H2TMlist <- list.files("../Predictions/ItemEffectsRandomCrit/Full/H2TM/", full.names = TRUE)
...
for(i in seq_along(explist4)){
  ...
  for(model in c("2HTM","MAgamSDT","M0gamSDT","DPgamSDT","EVgamSDT","UVgamSDT")){
    ...
    if(length(divs) > 0){
      ...
      saveRDS(Deviance[[1]] %>% mutate(experiment = exp)
              ,file=paste0("../ProcessedPredictions/ItemEffectsRandomCrit/Full/deviances_",model,"_",exp,"_Full.rds"))
    }
  }
...
EVSDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/Full/IndividualFull/", full.names = TRUE)
...
for(i in seq_along(d1)){
  ...
  for(model in c("2HTM","MAgamSDT","M0gamSDT","DPgamSDT","EVgamSDT","UVgamSDT")){
    ...
    if(length(divs) > 0){
      ...
      saveRDS(Deviance[[1]]# %>% mutate(experiment = exp)
              ,file=paste0("../ProcessedPredictions/ItemEffectsRandomCrit/IndividualFull/deviances_",model,"_",exp,"_Fullindividual.rds"))
    }
  }
...
EVSDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/KFCV/EVSDT/", full.names = TRUE)
UVSDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/KFCV/UVSDT/", full.names = TRUE)
DPSDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/KFCV/DPSDT/", full.names = TRUE)
M0SDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/KFCV/M0SDT/", full.names = TRUE)
H2TMlist <- list.files("../Predictions/ItemEffectsRandomCrit/KFCV/H2TM/", full.names = TRUE)
MASDTlist <- list.files("../Predictions/ItemEffectsRandomCrit/KFCV/MASDT/", full.names = TRUE)
...
for(i in seq_along(d1)){
  ...
  for(model in c("2HTM")){
    ...
    if(length(divs) > 0){
      ...
      saveRDS(Deviance
              ,file=paste0("../ProcessedPredictions/ItemEffectsRandomCrit/KFCV/deviances_",model,"_",exp,"_KFCV.rds"))
      saveRDS(DevianceInd
              ,file=paste0("../ProcessedPredictions/ItemEffectsRandomCrit/IndividualKFCV/deviances_",model,"_",exp,"_individualKFCV.rds"))
    }
  }
...
for(i in c(30,59,37,64,77,6,58,34,33,18,70,60)){#seq_along(d1)[1:78]){
    ...
    for(model in c("2HTM","MAgamSDT","M0gamSDT","DPgamSDT","EVgamSDT","UVgamSDT")){
      ...
      if(length(divs) > 0){
        ...
        divergtable <- T1s %>%
          mutate(pitemT1 = prop_itemT1,
                 pindT1 = prop_indT1,
                 partraw = n_indT1greater,
                 partraw_item = n_itemT1greater,
                 partall =n_exp,
                 pexpT1 = prop_expT1) %>%

          select(model,pitemT1,pindT1,pexpT1,partraw,partraw_item,partall) %>%
          #select(model,Div) %>%
          pivot_longer(names_to = "type",values_to = "value",
                       cols = c(pitemT1,pindT1,pexpT1,partraw,partraw_item,partall)) %>%
        ...
        saveRDS(divergtable,file=paste0("D:/HS01/ProcessedPredictions/ItemEffectsRandomCrit/Full/divergtable_",model,"_",exp,"_Full.rds"))
        saveRDS(divergtable2,file=paste0("D:/HS01/ProcessedPredictions/ItemEffectsRandomCrit/Full/diverg2table_",model,"_",exp,"_Full.rds"))
      }
    }
...
for (i in seq_along(d1)){
  ...
  for(model in c("EVgamSDT","UVgamSDT","DPgamSDT","2HTM","M0gamSDT","MAgamSDT")){
    ...
    LOPlist <-list.files(paste0("../Predictions/ItemEffectsRandomCrit/LOP/",modeln1,"/"), full.names = TRUE)
...
saveRDS(Divs1,file=paste0("../Predictions/ItemEffectsRandomCrit/processedLOP_Div.rds"))
...
filelist <-list.files("../ProcessedPredictions/ItemEffectsRandomCrit/Full/", full.names = TRUE)
...
saveRDS(pps2,"../AggPredictions/ItemEffectsRandomCrit/processedDev_ppp.rds")
...
filelist <- list.files("../Predictions/ItemEffectsRandomCrit/Full/LOOIC/", full.names = TRUE)
...
saveRDS(LOOIC
        ,file=paste0("../AggPredictions/ItemEffectsRandomCrit/processedLOOIC.rds"))
saveRDS(pareto
        ,file=paste0("../AggPredictions/ItemEffectsRandomCrit/processedLOOIC_paretok.rds"))
...
extractDevsInd <- function(div){

  Devs <- tibble(Dev = div$deviances$Dev$sumprob,
                 id = div$deviances$Dev$id) %>%
    mutate(model = div$information$model,
           type = div$information$type,
           exp = div$information$experiment
    )
}
...
extractT1s_ind<- function(div){
  ...
  T1s <- gb %>%
    mutate(
      itemT1 = itemT1preds - itemT1obs,
      indT1 = indT1preds - indT1obs) %>%
    group_by(id) %>%
    summarize(
      p_indT1 = mean(indT1 > 0),
      p_itemT1 = mean(itemT1 > 0)) %>%
    ungroup() %>%
    summarize(
      meanp_ind = mean(p_indT1),
      meanp_item = mean(p_itemT1),
      sdp_item = sd(p_itemT1),
      sdp_ind = sd(p_indT1),

      n_indT1greater = length(p_indT1[p_indT1 > .05]),
      n_itemT1greater = length(p_itemT1[p_itemT1 > .05]),
      n_exp = length(p_indT1)) %>%
    mutate(model = div$information$model) %>%
    mutate(
      prop_indT1 = n_indT1greater/n_exp,
      prop_itemT1 = n_itemT1greater/n_exp)
